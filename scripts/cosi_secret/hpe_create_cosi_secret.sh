...
create_secret()
{
    ...
    # Build kubectl command with base parameters
    kubectl_cmd="kubectl create secret generic $cosi_secret_name -n $cosi_secret_namespace \
...
opt_long="help,s3_credentials:,s3_endpoint:,glcp_credentials:,dscc_zone:,cluster_serial_number:,glcp_ca:,cosi_secret_name:,cosi_secret_namespace:"
...
option="$1"
    case $option in
        ...
        -c|--cluster_serial_number) # Enter the cluster serial number
            ...
            shift
            ;;
        --glcp_ca) # Enter the GLCP CA certificate
            glcp_ca=$2;
            shift
            ;;
