	...
	tests := []struct {
	...
	}{
		{
			...
			setupPatches: func() *gomonkey.Patches {
				...
				p = p.ApplyFuncReturn(getAccessToken, "eyJhbGciOiJSUzI1NiIsImtpZCI6IkxnLXg4TUxkTVNv", nil)
				...
				p = p.ApplyMethodReturn(user, "CreateS3User", "rd39awiMI", nil)
...
func TestServer_getAccessToken(t *testing.T) {
	...
	t.Run("Token retrieval successful", func(t *testing.T) {
		...
		expToken := "eyJhbGciOiJSUzI1NiIsImtpZCI6IkxnLXg4TUxkTVNv"
...
func getIAMCredentials() utils.IAMCredentials {
	return utils.IAMCredentials{
		GLCPUser:          "ac40eac7-9a06-4ccb-8575",
		GLCPUserSecretKey: "81656ff23d1f11efa0f6bghcabe03166b4d",
		DSCCZone:          "scdev03-xxx.xx.xxx.hpe.com",
		SystemId:          "DUMMY_SERIAL_NUMBER",
	}
}
